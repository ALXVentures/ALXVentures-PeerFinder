...
const StatusPage = () => {
  ...
  const submitUnpair = async () => {
    if (!unpairReason) { alert("Please select a reason"); return; }
    setLoadingUnpair(true);
    try {
      await axios.post(`${API_URL}/api/leave-group`, { user_id: userId, reason: unpairReason, delete_profile: unpairAction === 'delete' });
      ...
      if (unpairAction === 'delete') {
         ...
         setFeedbackModal({ isOpen: true, title: 'Unpaired Successfully', message: 'You have been unpaired and placed back in the queue.', type: 'success', redirect: null });
         await fetchStatus(); 
      }
    } catch (err) { alert("Error processing request."); } finally { setLoadingUnpair(false); }
  };
    ...
    <div style={styles.container}>
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} style={styles.card}>
        ...
        <div style={styles.body}>
          {isDuplicate && (
            <div style={styles.duplicateWarning}>⚠️ <strong>You are already registered!</strong><br/>Here is your current status below.</div>
          )}

          {status.matched ? (
            <div>
              <div style={styles.successBadge}>✓ MATCHED</div>
              <h3 style={{ color: colors.primary.berkeleyBlue }}>Your Connection(s):</h3>
              ...
              {status.group.map((peer, idx) => (
                <div key={idx} style={{ background: 'white', padding: '15px', borderRadius: '10px', marginBottom: '10px', boxShadow: '0 2px 10px rgba(0,0,0,0.05)', borderLeft: `5px solid ${colors.secondary.electricBlue}` }}>
                  <p style={{ margin: '0 0 5px 0', fontWeight: 'bold', fontSize: '1.1rem' }}>{peer.name} <span style={{fontSize:'0.8rem', background:'#eee', padding:'2px 8px', borderRadius:'10px'}}>{peer.program}</span></p>
                  <p style={{ margin: '0 0 5px 0', color: '#555', fontSize: '0.9rem' }}>📧 {peer.email}</p>
                  {peer.skill_type && peer.skill_type !== 'None' && <p style={{ margin: '0 0 5px 0', color: '#555', fontSize: '0.9rem' }}>💼 Skill: <strong>{peer.skill_type}</strong></p>}
              ...
              <div style={{ marginTop: '20px', background: '#e3f2fd', padding: '20px', borderRadius: '10px', textAlign: 'center', border: '1px solid #b8daff' }}>
                  ...
                  <a href={`https://meet.jit.si/ALX-Ventures-${status.user?.real_id || status.group[0]?.name.replace(/\s/g,'')}`} target="_blank" rel="noreferrer" style={{ background: '#0056b3', color: 'white', padding: '12px 25px', borderRadius: '30px', textDecoration: 'none', fontWeight: 'bold', fontSize: '1rem', display: 'inline-block' }}>Join Video Meeting Now</a>
              </div>
            ...
            <div style={{ textAlign: 'center' }}>
              ...
              {matchMessage && ( <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={styles.noticeBox}>{matchMessage}</motion.div> )}
            ...
            <motion.div initial={{ y: 50 }} animate={{ y: 0 }} style={styles.modalContent}>
              ...
              <div style={{ textAlign: 'left', marginBottom: '20px' }}>
                  ...
                  <select style={{ width: '100%', padding: '10px', marginTop: '5px', borderRadius: '5px', border: '1px solid #ccc' }} onChange={(e) => setUnpairReason(e.target.value)} value={unpairReason}>
                      <option value="">-- Select Reason --</option><option value="Ghosting">Partner didn't show up</option><option value="Schedule Conflict">Schedule Conflict</option><option value="Found someone else">Found someone else</option><option value="Other">Other</option>
                  </select>
              </div>
